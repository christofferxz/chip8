...
typedef struct {
  ...
  bool pixel_outlines;  // Draw pixel outlines Y\N;
...
typedef struct {
  ...
  uint8_t ram[4096];  // (0x1000) memory locations, all of which are 8 bits
  ...
  uint16_t PC;  // Pogram Counter
  uint8_t delay_timer;  // Decrements at 60hz when >0
  uint8_t sound_timer; // Decrements at 60hz and plays tone wehn >0
...
bool initialize_sdl(sdl_t *sdl, const config_t config) {
  // Initialize SDL video, audio and timer
...
bool set_config_from_args(config_t *config, const int argc, char **argv) {
  // Set defaults values for config
  *config = (config_t) {
...
bool init_chip8(chip8_t *chip8, const char rom_name[]) {
  ...
  // Load font
  memcpy(&chip8->ram[0], font, sizeof(font));  // Copy the font to the start of the RAM
  ...
  const size_t max_size = sizeof chip8->ram - entry_point;  // Max size = RAM(4096) - entry_point(512)
  rewind(rom);

  // Check if ROM size is not too big for available RAM
  ...
  // Check if was able to read ROM file
  ...
  chip8->rom_name = rom_name; // Set current loaded ROM
  chip8->stack_ptr = &chip8->stack[0]; // Set stack pointer to default stack
...
void update_screen(const sdl_t sdl, const config_t config, const chip8_t chip8) {
  ...
  // Loop through display pixexls, draw a rectangle per pixel to the SDL window
  for (uint32_t i = 0; i < sizeof chip8.display; i++) {
    // Translate ld index i value to 2D XX/Y coordinates
    // X = i % window width
    // Y = i / window width
    ...
    switch (event.type) {
      ...
      case SDL_KEYDOWN:
        switch (event.key.keysym.sym) {
          case SDLK_ESCAPE:
            ...
            return;
          case SDLK_SPACE:
            // Space bar; Handle pause and resume
          ...
          case SDLK_2: chip8->keypad[0x1] = true; break;
          case SDLK_3: chip8->keypad[0x1] = true; break;
      ...
      case SDL_KEYUP:
        switch (event.key.keysym.sym) {
          ...
          case SDLK_2: chip8->keypad[0x1] = false; break;
          case SDLK_3: chip8->keypad[0x1] = false; break;
  ...
  switch ((chip8->inst.opcode >> 12) & 0x0F) {
    case 0x00:
      if (chip8->inst.NN == 0xE0) {
        ...
        printf("Unimplemented/invalid opcode, may be 0xNNN for calling machine code routine for RCA1802\n");
      };
      break;
    case 0x01:
    ...
    case 0x02:
      ...
      //  and set program to subroutine address so that the next opcode
    ...
    case 0x04:
      ...
      break;
    case 0x05:
      // 0x5XY0: Check if VX != NN, if so, skip the next instruction
    ...
    case 0x08:
      switch (chip8->inst.N) {
        case 0:
          ...
          printf("Set register V%XX = V%X (0x%02X)\n",
        ...
        case 3:
         // 0x8XY3: Set register VX ^= VY
          ...
          break;
        case 4:
          // 0x8XY4: Set register VX += VY, set VF to 1 if there is not a borrow (result is positive/0)
        ...
        case 6:
          ...
          break;
        case 7:
          // 0x8XY7: Set VX = VY - VX, set VF to 1 if there is not a borrow (result is positive/0)
        ...
        default:
          ...
          printf("Wrong/unimplemented opcode\n");
          break;
      };
      break;
    case 0x09:
      ...
      break;
    case 0x0A:
      // 0xANNN: Set indexregister I to NNN
      printf("Set I to NNN (0x%04X)\n",
            chip8->inst.NNN);
      break;
    case 0x0B:
      // 0xBNNN: Jump to V0 + NNN
      printf("Set PC to V0 (0x%02X) + NNN (0%04x); Result PC = 0x%04X\n",
    ...
    case 0x0C:
      ...
      printf("Set V%X = rand() %% 256 & NN (0x%02X)\n",
              chip8->inst.X, chip8->inst.NN);
    case 0x0D:
      // 0xDXYN: Draw N-height sprite at coordinate VX, VY; Read from memory location I;
      //  Screen pixels are XOR'd with sprite bits,
      //  VF (Carry flag) is set if any screen pixexl are set off; This is useful
      //  for collision detection or other reasons.
      printf("Draw N (%u) height sprite at coords V%X (0x%02X), V%X (0x%02X) "
              "from memory location I (0x%04X). Set VF = 1 if any pixel are turned off.\n",
    ...
    case 0x0F:
      switch (chip8->inst.NN) {
        case 0x0A:
          ...
          break;
        case 0x1E:
          // 0xFX1EL I += VX; Add VX to register I. For non-Amiga CHIP8, does not affect VF
        ...
        case 0x33:
          ...
          //  I = hundred's place, I+! = ten's place, I+2 = one's place
        ...
        case 0x55:
          ...
          break;  
        case 0x65:
          // 0xFX55: Register dump V0-VX inclusive from memory offset from I
          //  SCHIP does not increment I, CHIP8 does increment I
          printf("Register load V0-V%X (0x%02X) inclusive at memory from I (0x%04X)\n",
                  chip8->inst.X, chip8->V[chip8->inst.X], chip8->I);
          break;
        default:
          printf("Unimplemented or invalid 0xFX opcode\n");
          break;
      };
      break;
    default:
    ...
    case 0x08:
      switch (chip8->inst.N) {
        ...
        case 0xE:
          ...
          break;
        default:
          break;  // Wrong/unimplemented opcode
      };
      break;
    case 0x09:
      // 0x9XY0: Check if VX != VY; Skip next instruction if so
    ...
    case 0x0D:
      ...
      for (uint8_t i = 0; i < chip8->inst.N; i++) {
        ...
        for (int8_t j = 7; j >= 0; j--) {
          // If sprite pixxel/bit is on and display pixel is on, set carry flag
...
int main(int argc, char **argv) {
  ...
  while (chip8.state != QUIT)  {
    ...
    // Delay for approximately 60hz/60fps (16.67ms) or acutal time elapsed
